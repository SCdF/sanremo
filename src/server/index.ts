...
app.get('/api/deployment', async (req, res) => {
  ...
  if (process.env.NODE_ENV === 'production') {
    toReturn.deploy_created_at = process.env.RAILWAY_DEPLOYMENT_TIMESTAMP || process.env.HEROKU_RELEASE_CREATED_AT;
    toReturn.deploy_version = process.env.RAILWAY_DEPLOYMENT_ID || process.env.HEROKU_RELEASE_VERSION;
