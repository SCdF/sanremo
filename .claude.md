...
### ⚠️ CRITICAL: Always Create Branch FIRST

**BEFORE making ANY code changes:**

1. **CHECK current branch**: `git branch --show-current`
2. **IF on `main`**: STOP and create a branch FIRST:
   ```bash
   git checkout -b descriptive-branch-name
   ```
3. **THEN** make code changes
4. **THEN** run `yarn check && yarn test`
5. **THEN** commit and push

**This is NON-NEGOTIABLE.** Never commit code changes directly to `main`.

### Branch Strategy
- **ALWAYS work on branches for code changes**
- If not on a branch when making code changes, create one first
- Branch naming: Use descriptive names like `fix-skipped-tests` or `feat-new-component`
- **I can push branches myself** after pre-commit checks pass
- **I should create a draft PR** using `gh pr create --draft` after pushing
- **I should check CI status** using `gh pr checks` and report results
...
5. Report results to the user with PR link and CI status
...
# 4. Create draft PR and check CI
gh pr create --draft --title "..." --body "..."
gh pr checks  # Wait for and verify CI passes
```
